  ...
  namespace :common do
    ...
    task :install do |t|
      ...
      unless File.exists?(marker_file)
        sh "pip install -r requirements.txt --use-mirrors --download-cache $HOME/.pip-cache-$TRAVIS_PYTHON_VERSION 2>&1 >> /tmp/ci.log"
        sh "pip install -r test-$TRAVIS_PYTHON_VERSION-requirements.txt --use-mirrors --download-cache $HOME/.pip-cache-$TRAVIS_PYTHON_VERSION 2>&1 >> /tmp/ci.log"
