...
namespace :ci do
  namespace :mongo do
    ...
    task :before_script => ['ci:common:before_script'] do
      ...
      sleep_for(15)
      sh %Q{sudo mongo --verbose --eval "printjson(db.serverStatus())" 'localhost:37017' >> /tmp/mongo.log}
      sh %Q{sudo mongo --verbose --eval "printjson(db.serverStatus())" 'localhost:37018' >> /tmp/mongo.log}
      sh %Q{sudo mongo --verbose --eval "printjson(rs.initiate()); printjson(rs.conf());" 'localhost:37017' >> /tmp/mongo.log}
      sleep_for(30)
